...
import { 
  ...
  ScrollView,
  ActivityIndicator,
  SafeAreaView // Usamos SafeAreaView para el WebView
} from 'react-native';
import { WebView } from 'react-native-webview';

// Función auxiliar para codificación Base64
const base64Encode = (str: string): string => {
    return typeof btoa === 'function' ? btoa(str) : 'ERROR_BTOA_UNDEFINED';
};


// CARGAR lOGINSCREEN QUE ESTA EN SRC/IVOO/SCREENS/LOGINSCREEN.TSX
...
<request>
    ...
    <cedula_rif>${identificacion}</cedula_rif>
        ...
        if (response.ok && responseData.trim().match(/^\d+$/)) { 
            ...
            const url: string = `https://paytest.megasoft.com.ve/action/paymentgatewayuniversal-data?control=${referenciaObtenida}`;
  ...
  // ------------------------------------------------------------------
  // 🟢 RENDERIZADO CONDICIONAL DEL WEBVIEW
  // ------------------------------------------------------------------
  if (paymentUrl) {
    return (
