...
export const registerUser = createAsyncThunk(
  ...
  async (data: RegisterRequest, {rejectWithValue}) => {
    try {
      ...
      // Validar que la respuesta tenga token y usuario
      if (!response.token || !response.user) {
        throw new Error(
          'Respuesta inválida del servidor: faltan token o usuario',
        );
      }

      // Guardar token y usuario
      console.log('[RegisterSlice] Guardando token y datos del usuario');
